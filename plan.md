...
本文档原先内嵌了一份逐日更新的开发流水账（约 500 行）。该部分内容已按性质拆分：

- **已发布版本的变更** → [CHANGELOG.md](CHANGELOG.md)
- **逐次提交的完整记录** → `git log`（提交信息遵循 Conventional Commits）
- **本文件只保留仍然有效的架构设计与技术决策**（上文各章）

之所以删除流水账，是因为它记录的目录结构（`src/web_server.cpp`、`include/web_server.h`、`profiler_lib` 目标、`profiler::registerHttpHandlers()`、`web/` 目录等）在后续重构中已被删除或改名，继续保留会误导读者，而且与 CHANGELOG.md 重复。

当前架构请以 [项目结构](README.md#项目结构) 与 `CMakeLists.txt` 中的 `profiler_core` / `profiler_web` 目标为准。
